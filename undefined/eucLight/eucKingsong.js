...
euc.tmp.one=function(inpk){
	...
	amp=amp/100; if (amp!=dash.live.amp) { dash.live.amp=amp;euc.emit("amp",amp); }				
};
...
euc.conn=function(mac){
	...
	//read
		...
		c.on('characteristicvaluechanged', function(event) {
            ...
            if (inpk[0]==188||inpk[0]==65){if (euc.dbg)  print("drop",inpk); euc.wri("start"); return;}
            //if (euc.busy||inpk[0]==188){if (euc.dbg)  print("drop",inpk); return;}
			if (ew.is.bt==4&&global.euc.proxy&&global.euc.proxy.state) 	euc.proxy.w(event.target.value.buffer);
			euc.alert=0;
	...
	//write
		...
		c.startNotifications().then(function() {
				...
				if (euc.dbg)  print("EUC: ks initialized");
				setTimeout(() => {
					if (!euc.gatC.value) {if (euc.dbg)  print("EUC: ks init value not found"); euc.wri("start");}
				}, 1000);
				euc.state="READY";
				euc.run=1;
