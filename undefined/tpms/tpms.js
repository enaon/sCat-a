					...
					if (mac[1]+mac[2] == "eaca") {
						...
						if ( tpms.def.allowNew || tpms.def.list[id] ) {
							...
							if (dev.psi<tpms.def.list[id].lowP) {
								...
								notify.alert("euc",{"src":"TPMS","title":id,"body":"LOW PRESSURE."+"  "+dev[tpms.def.metric]+" "+tpms.def.metric+"  "},1);
							}else if (tpms.def.list[id].hiP <=dev.psi) {
								alrm=2;
								if (euc.state=="READY") euc.dash.alrt.warn.txt="HI PRESSURE";
								notify.alert("euc",{"src":"TPMS","title":id,"body":"HI PRESSURE."+"  "+dev[tpms.def.metric]+" "+tpms.def.metric+"  "},1);
							} else alrm=0;
