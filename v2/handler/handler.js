...
if (process.env.BOARD != "DSD6") {
    ...
    if (ew.def.size) {UI.size.txt=ew.def.size;UI.size.sca=ew.def.size*(process.env.BOARD=="BANGLEJS2"?0.9:1.1);}

    

}
else if (require('Storage').read("handler_acc_dsd6") && ew.def.acc) eval(require('Storage').read("handler_acc_dsd6"));
