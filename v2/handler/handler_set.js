...
ew.do.reset.settings=function() {
	...
	"name":"eucWatch_v2.0","touchtype":"0","acctype":"0","hr24":1,"prxy":0,"timezone":"0","woe":1,"wob":1,"rfTX":-4,"cli":1,"hid":0,"gb":0,"atc":0,"acc":0,"hidT":"media","bri":2,"buzz":1,"bpp":4,"info":1,"txt":1};
...
ew.do.update.bluetooth=function(){ 
	try {
		...
		if (ew.def.hid&&!require("Storage").read("hid")) {ew.def.hid=0; return;}
		if (ew.def.hid) {
			ew.def.cli=0;ew.def.gb=0;
			NRF.setAdvertising({}, { name:ew.def.name+"-HiD-"+ew.def.hid,connectable:true });
			NRF.setAddress(NRF.getAddress().substr(0,15)+ew.def.hid+"b random");
			if (ew.is.hidM==undefined) {
				...
				ew.is.hidM.do= function (i) {
					try {
					   ...
					   if (ew.dbg)	notify.alert("info",{"src":"DBG","title":"HID:","body":"YOUAREFAST"},1);	
					}
				}
			}
	...
	*/
	  	}else if (!ew.def.hid &&ew.is.hidM!=undefined) {
