...
scata = {
  ...
  count: function(i) {
    ...
    scata.tid.cntdn = setInterval(() => {
      ...
      if (v == 90 && ew.def.name != "eL-6f") { if (scata.state.dbg) print("scata-Info: sleep counter");
...
//recovery
  ...
  silicone_return3: ()=> { scata.go({ one: 0.1, two: 0, act: "Leveling", next: "silicone_return4", speed: 30 }); },
  silicone_return4: ()=> { scata.go({ one: 0.1, two: 0, act: "Leveling", next: "silicone_return5", speed: 80 }); },
  silicone_return5: ()=> { scata.go({ one: 0.1, two: 0, act: "Leveling", next: "silicone_return6", speed: 80 }); },
  silicone_return6: ()=> { scata.go({ one: 0.8, two: 0.65, act: "Leveling", next: "lock", speed: 100 }); },
  // empty sand
  ...
  //
  sleep: function() {
    if (ew.tid.scata) {clearTimeout(ew.tid.scata);ew.tid.scata = 0;}
    //scata.state.busy = 0;
  ...
  wake: function(i, e) {
    ...
    scata.tid.move = setInterval(() => {
      if (4.5 <= ew.is.ondcVoltage()) {
        if (scata.state.dbg) print("scata-Info: power is on");
        clearInterval(scata.tid.move);
        scata.tid.move = 0;
        scata.volt.base = ew.is.ondcVoltage();
        if (i) { if (scata.state.dbg) print("scata-Info: starting", i, e ? e : "");
          scata[i](e ? e : ""); }
      }
      else {
        if (scata.state.dbg) print("scata-Info: waiting for power");
        digitalPulse(D23, 1, 1000);
      }
    }, 2000);
  }
};
...
acc.on("action", (x, y) => {
  ...
  if (y == 1 || y == 2) scata.state.tap = 5;
    ...
    if (scata.state.dbg) print("scata-Info: acc event", x, y,"ignored");
    return;
  }
  //if (ew.tid.scata) {
  ...
  if (scata.state.busy || BTN1.read()) { if (scata.state.dbg) print("scata-Info: acc event", x, y,"ignored"); return;}
  else  scata.count();
});
//cron event
cron.on('hour',() => {
  ...
  if ( scata.state.busy) {
    if (scata.state.dbg) print("scata-Info: cron event ignored");
    return;
  }
  if (ew.is.ondcVoltage(1)<=50){
    if (scata.state.dbg) print("scata-Info: cron event power on");
    scata.wake();
  }else if  (100<=ew.is.ondcVoltage(1)&& ew.is.ondcVoltage(1)<=120){
    if (scata.state.dbg) print("scata-Info: cron event power off");
    scata.sleep();
  }
});
//init
